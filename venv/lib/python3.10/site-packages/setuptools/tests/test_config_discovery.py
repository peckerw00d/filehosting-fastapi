...
class TestDiscoverPackagesAndPyModules:
    ...
    def test_project(self, tmp_path, circumstance):
        ...
        (tmp_path / "build/bdist.linux-x86_64/test_file.py").touch()
        (tmp_path / "build/lib/__init__.py").touch()
        (tmp_path / "build/lib/test_file.py").touch()
        (tmp_path / "dist").mkdir()
        (tmp_path / "dist/test_file.py").touch()
